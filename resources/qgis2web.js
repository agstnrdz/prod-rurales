...
map.getView().fit([-7531899.858046, -5767342.372953, -7497924.771108, -5737553.556653], map.getSize());
...
var Title = new ol.control.Control({
    element: (() => {
        ...
        titleElement.innerHTML = '<h2 class="project-title">Productores de la agricultura familiar</h2>';
        return titleElement;
    })(),
    ...
    element: (() => {
        ...
        if (27 > 240) {
            ...
            window.showAbstract = function() {
                ...
                linkElement.innerHTML = 'Actualizado a julio de 2026';
            }
...
//geolocate
	...
	geolocateButton.title = 'Mostrar mi ubicación en tiempo real';
	...
	// círculo de precisión GPS, dibujado en coordenadas del mapa
	const accuracyFeature = new ol.Feature();
	const geolocateOverlay = new ol.layer.Vector({
		source: new ol.source.Vector({ features: [accuracyFeature] }),
		style: new ol.style.Style({
			fill: new ol.style.Fill({ color: 'rgba(51, 153, 204, 0.15)' }),
			stroke: new ol.style.Stroke({ color: 'rgba(51, 153, 204, 0.7)', width: 1.5 }),
		}),
	});

	// marcador "estás aquí", como overlay HTML para poder animarlo (pulso)
	// y rotarlo con CSS según el rumbo de movimiento
	const locationMarkerEl = document.createElement('div');
	locationMarkerEl.className = 'location-marker';
	locationMarkerEl.innerHTML =
		'<div class="location-marker-pulse"></div>' +
		'<div class="location-marker-arrow"></div>' +
	...
	// lectura de coordenadas/precisión en vivo, para volcar a la planilla papel
	// (se agrega junto a los demás controles de la esquina superior izquierda)
	const geoStatusEl = document.createElement('div');
	geoStatusEl.className = 'ol-unselectable ol-control geo-status';
	geoStatusEl.title = 'Tocar para copiar las coordenadas';
	geoStatusEl.style.display = 'none';
	document.getElementById('top-left-container').appendChild(geoStatusEl);
	geoStatusEl.addEventListener('click', function () {
		if (!geoStatusEl.dataset.coords || !navigator.clipboard) {
			return;
		}
		navigator.clipboard.writeText(geoStatusEl.dataset.coords).then(function () {
			var original = geoStatusEl.innerHTML;
			geoStatusEl.classList.add('copied');
			geoStatusEl.innerHTML = 'Coordenadas copiadas';
			setTimeout(function () {
				geoStatusEl.classList.remove('copied');
				geoStatusEl.innerHTML = original;
			}, 1200);
		});
	});
	...
	geolocation.on('change:position', function () {
		...
		positionOverlay.setPosition(coords);

		// apunta la flecha en el sentido de la marcha cuando el dispositivo lo reporta
		const heading = geolocation.getHeading();
		const arrowEl = locationMarkerEl.querySelector('.location-marker-arrow');
		if (typeof heading === 'number' && !isNaN(heading)) {
			arrowEl.style.display = 'block';
			arrowEl.style.transform = 'rotate(' + (heading * 180 / Math.PI) + 'deg)';
		} else {
		...
		geoStatusEl.dataset.coords = lonLat[1].toFixed(6) + ', ' + lonLat[0].toFixed(6);
		geoStatusEl.innerHTML = 'Lat ' + lonLat[1].toFixed(5) + '&nbsp;&nbsp;Lon ' + lonLat[0].toFixed(5) +
			(accuracy ? '&nbsp;&nbsp;(±' + Math.round(accuracy) + ' m)' : '');

		// modo "seguir": mientras está activo, el mapa se recentra en cada actualización
		if (isTracking) {
	...
	geolocation.on('error', function (error) {
		...
		geoStatusEl.classList.add('geo-error');
		geoStatusEl.innerHTML = 'No se pudo obtener tu ubicación: ' + error.message;
	...
	function handleGeolocate() {
		if (isTracking) {
			...
			geoStatusEl.classList.remove('geo-error');
			...
			if (pos) {
				map.getView().animate({ center: pos, duration: 300 });
			}
		}
	}
